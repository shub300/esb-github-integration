...
class CreatePlatformOrderShipmentsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('platform_order_shipments', function (Blueprint $table) {
            ...
            $table->bigInteger('platform_order_id')->nullable()->index('cascade_platform_order_shipments')->comment('your order table primary id');
            $table->string('order_id', 30)->nullable()->index('order_id')->comment('API primary id of the order');
            $table->integer('shipment_sequence_number')->default(0)->comment('api shipment sequence number');
            ...
            $table->integer('event_owner_id')->nullable()->comment('basically a api_customer id/employee_id');
    ...
    {
        Schema::dropIfExists('platform_order_shipments');
    }
}
