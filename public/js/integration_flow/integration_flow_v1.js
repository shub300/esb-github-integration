...
function storeMapping(type = 1)
{
    ...
    if (countIdentMapField > 0) {
        validateIdentity(validErrStatus,identData)
    }
    ...
    if (countFMsection > 0) {
        for (let i = 1; i <= countFMsection; i++) {
            ...
            if((sourceFieldID) || (destFieldID))
            {
                formData += `{"data_map_type":"field","platform_object_id":"${platform_object_id}","mapping_type":"regular","platform_workflow_rule_id":"${pfwfrID}","source_row_id":"${sourceFieldID}","destination_row_id":"${destFieldID}","custom_data":"","status":1,"editId":"${editId}"},`;
            }
...
function getFlowData() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            document.getElementById('FlowDataSection').innerHTML = flowData;

            if(data.data.length==0)
            {
                let emptyData=`<tr><td colspan="4" style="text-align:center;">No Connection Found</td></tr>`;
                document.getElementById('FlowDataSection').innerHTML = emptyData;
            }
...
//on off switch
function switchAction(pfwfrID, userIntegId) {
    var csrf_token = $('meta[name="csrf-token"]').attr('content');
    let status = "";
    var fixedUrl = $("#AjaxCallUrl").val();
    ...
    if (!ischecked) {
        status = 0;
        flow_off_confirmation("Confirm!","Are you sure that you want to off this  flow?",userIntegId,pfwfrID,status,fixedUrl,csrf_token);
    }
    else {
        status = 1;
...
function confirmationDelFile(title,content,name,platform,editid,userIntegId,file,type,isConnectionPage){
 ...
 function flow_off_confirmation(title,content,userIntegId,pfwfrID,status,fixedUrl,csrf_token){
