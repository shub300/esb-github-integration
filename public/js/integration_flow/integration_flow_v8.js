...
function storeMapping(type = 1)
{
    ...
    if (countFMsection > 0) {
        for (let i = 1; i <= countFMsection; i++) {
            ...
            for (let j = 1; j <= CountFieldInSection; j++) {
                let s = `.Section${i} > .col-md-5 > .sourceField${j}`;
                sourceFieldID = $(s).val();
                let editId = $(s).attr("data-editId");
                platform_object_id = $(s).attr("alt");
                data_map_type = $(s).attr("data_map_type");
                mapping_type = $(s).attr("mapping_type");
                let d = `.Section${i}  > .col-md-5 > .destField${j}`;
                destFieldID = $(d).val();
    ...
    //push unique validation data in form data
    let list_uniqueMappingCheck = JSON.parse($("#list_uniqueMappingCheck_object").val());
    if(list_uniqueMappingCheck) {
        form_data.append("list_uniqueMappingCheck", list_uniqueMappingCheck);
    }
        ...
        success: function (data) {
            ...
            $.each(data.data, function (k, v) {
                ...
                if (v.status == 1) {
                    ...
                    //enable_reauth_button
                    if(v.enable_reauth_button=="YES") {
                        //enable alert
                        if(v.alertMsg) {
                            $("#reauthAlertMsg").html(v.alertMsg);
                            $("#reauthAlert").css("display", "block");
                        }
                        action +=`&nbsp;
                            <a href="${fixedUrl}/snowflake-re-auth-token/${userIntegId}" target="_top" 
                                onclick="window.open( this.href, 'newwindow', 'width=625,height=600'); return false;"
                                type="button" class="btn btn-warning btn-sm accountReauth warning-btn-style" data-id='${v.id}' data-platformid='${v.platform_id}' data-platform_name='${v.account_name}'>
...
function addOtherMap(pobjName)
{
    ...
    {   
        ...
        //clear dummy value also
        if(pobjName=="inventory_warehouse_TO_ip_s3_access_path"){
            $(newClass).find(".sourceSelect_dummy").val("");   
        }
    }
    else
    {
    ...
    if(dest_input_type !="select-one")
    {
        ...
        //clear dummy value also
        if(pobjName=="inventory_warehouse_TO_ip_s3_access_path"){
            $(newClass).find(".destSelect_dummy").val("");   
        }
    }
    else
    {
...
$(document).on('click', '.generateApplicationUrl', function(){
    const otherMapValidationArr = JSON.parse($("#otherMapValidationArr").val());
    $.each(otherMapValidationArr,function(key,pobjName){
        if(pobjName=="inventory_warehouse_TO_ip_s3_access_path") {
            ...
            $("."+pobjName).find(".col-md-12").each(function(k,v){
                ...
                let sourceFieldID=null; let destFieldID=null;  
                sourceFieldID = $( this ).find(".sourceSelect").val();
                destFieldID = $( this ).find(".destSelect").val();
                pfwfrID = $( this ).find(".sourceSelect").attr('name');

                //check source side mapping
                if (sourceFieldID == "" || sourceFieldID == null) {
                    if($( this ).find(".sourceSelect").prop('required')){
                        toastr.error('Source Mapping Fields must be Required for Url Generation.');
                        valid_error = true;
                        return true;
                    }
                }
                if(sourceFieldID && valid_error==false)
                {
                    // let newObject = {
                    //     'object_name': 'ip_s3_access_path',
                    //     'source_row_id': sourceFieldID,
                    //     'destination_row_id': '',
                    //     'custom_data': destFieldID
                    // };     
                    // urlFormData.push(newObject);
                    urlFormData.push(sourceFieldID);

                }
            })

            if(urlFormData && valid_error==false)
            {   
                let csrf_token = $('meta[name="csrf-token"]').attr('content');
                let fixedUrl = $("#AjaxCallUrl").val();
                let userIntegId = $("#input_IntegPlateformId").val();
                ...
                $.ajax({
                    type: 'post',
                    url: fixedUrl+'/load_ip_project_application_url',
                    data: {
                        'pobjName':'ip_s3_access_path',
                        'pfwfrID' : pfwfrID,
                        'userIntegId': userIntegId,
                        '_token': csrf_token,
                    ...
                    success: function (response) {
                        ...
                        if(response.status_code==1){
                            ...
                            if((response.data) && (response.data.length > 0))
                            {
                                ...
                                $("."+pobjName).find(".col-md-12").each(function(k,v){
                                    //set in hiddne field
                                    $( this ).find(".destSelect").val(response.data[i].custom_data);
                                    //set in dummy div
                                    let formated_data = response.data[i].custom_data;
                                    formated_data = formated_data.replace("{", "");
                                    formated_data = formated_data.replace("}", "");
                                    formated_data = formated_data.replace(",", "\n");
                                    $( this ).find(".destSelect_dammy").val(formated_data);
                                    i++;
                                })
                            }
