...
function connectBlackLine() {

    $('#mdlBlackLine').modal('toggle');
    pdata = new FormData($(".blacklineform")[0]);
    showOverlay();
    $('.connect-now').prop('disabled', true);
    $.ajax({
        url: baseUrl + "/ConnectBlackLineAuth",
        data: pdata,
        dataType: "json",
        async: true,
        type: "post",
        ...
        success: function(res) {
            ...
            if (res.status_code == "1") {
                ...
                retrievePlatformAccounts('blackline', 'destination');
            } else {
...
$(document.body).on('click', '#btnSubmitBlackline', function() {
    valid = true;
    $('.field_error').hide();
