...
$(document.body).on('click', '#btnSubmitShipRush', function() {
    valid = true;
    $('.err_mgs').hide();
    ...
    if (!checkvalue($('#user_token'))) {
        showErrorAndFocus($('#user_token'));
        valid = false;
    }
    if (!checkvalue($('#developer_token'))) {
        showErrorAndFocus($('#developer_token'));
        valid = false;
    }
    if (!checkvalue($('#shiprush_version'))) {
        showErrorAndFocus($('#shiprush_version'));
        valid = false;
    }
    if (!valid) {
        return false;
    }
    $('#mdlShipRush').modal('toggle');
});

function connectShipRush(){
    $('#mdlShipRush').modal('toggle');

    pdata = new FormData($(".shiprush_connect_form")[0]);
    showOverlay();
    $('.connect-now').prop('disabled',true);
    $.ajax({
        url: baseUrl + "/ConnectShipRush",
        data: pdata,
        dataType: "json",
        async: true,
        type: "post",
        ...
        success: function (res) {
            ...
            if (res.status_code == "1") {
                ...
                retrievePlatformAccounts('shiprush', 'destination');
            }
            else{
