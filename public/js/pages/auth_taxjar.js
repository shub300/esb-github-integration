...
$(document.body).on('click', '#btnSubmitTaxJar', function(){
    valid = true;
    $('.err_mgs').hide();
    if(!checkValue($('#access_token'))){
        showErrorAndFocus($('#access_token'));
        valid = false;
	}
    ...
    $('#mdlTaxJar').modal('toggle');
});

function connectTaxJar()
{
    $('#mdlTaxJar').modal('toggle');
	
    pdata = new FormData($(".TaxJar_connect_form")[0]);
    showOverlay();
    $('.connect-now').prop('disabled',true);
    $.ajax({
        url: baseUrl + "/ConnectTaxJar",
        data: pdata,
        dataType: "json",
        async: true,
        type: "post",
        ...
        success: function(res){
            ...
            if(res.status_code == "1")
            {
                ...
                retrievePlatformAccounts('TaxJar', 'destination');
			}
            else
            {
